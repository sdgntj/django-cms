...
class AbstractPagePermission(models.Model):
    ...
    """
    # who:
    user = models.ForeignKey(getattr(settings, 'AUTH_USER_MODEL', 'auth.User'), verbose_name=_("user"), blank=True, null=True)
...
class PageUserGroup(Group):
    ...
    """
    created_by = models.ForeignKey(getattr(settings, 'AUTH_USER_MODEL', 'auth.User'), related_name="created_usergroups")
